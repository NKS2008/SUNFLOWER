/* ===========================================================
   ...
   Flower type is selected by setting `window.FLOWER_TYPE`
   before this script loads: 'sunflower', 'lily', or 'tulip'
   (default).
   =========================================================== */

const FLOWER_TYPE = window.FLOWER_TYPE || 'tulip';

// Pollen / sparkle particle color range per flower type
const PARTICLE_HUE_RANGE =
    FLOWER_TYPE === 'sunflower' ? [40, 55] :
    FLOWER_TYPE === 'lily' ? [320, 345] :
    [330, 370];
...
class Particle {
    ...
    reset(initial = false) {
        ...
        const [hMin, hMax] = PARTICLE_HUE_RANGE;
        this.hue = hMin + Math.random() * (hMax - hMin);
...
class FlowerBloomApp {
    ...
    // ----- Flower Head (dispatches per flower type) -----
    drawFlowerHead(cx, cy, bloom, windAngle, scale) {
        if (this.flowerType === 'sunflower') {
            this.drawSunflowerHead(cx, cy, bloom, windAngle, scale);
        } else if (this.flowerType === 'lily') {
            this.drawLilyHead(cx, cy, bloom, windAngle, scale);
        } else {
            this.drawTulipHead(cx, cy, bloom, windAngle, scale);
        }
    }

    // ----- Flower Head (Tulip facing upward) -----
    drawTulipHead(cx, cy, bloom, windAngle, scale) {
    ...
    // ----- Flower Head (Sunflower — radial ray florets around a dark disc) -----
    drawSunflowerHead(cx, cy, bloom, windAngle, scale) {
        const bloomScaleFactor = 1.0 + bloom * 0.18;
        const adjustedScale = scale * bloomScaleFactor;
        const ctx = this.ctx;
        ...
        // Ambient golden glow
        const glowR = (60 + bloom * 130) * adjustedScale;
        if (bloom > 0.02) {
            const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, glowR);
            glow.addColorStop(0, `rgba(255, 205, 70, ${0.42 * bloom})`);
            glow.addColorStop(0.5, `rgba(255, 165, 40, ${0.18 * bloom})`);
        ...
        const petalCount = 18;
        const maxPetalLen = 72 * adjustedScale;
        const petalWidth = 15 * adjustedScale;
        const centerR = (20 + bloom * 5) * adjustedScale;

        // Back layer of ray petals (slightly offset, darker) for depth
        for (let i = 0; i < petalCount; i++) {
            const baseAngle = ((i + 0.5) / petalCount) * Math.PI * 2;
            const flutter = this.noise.get(this.time * 1.0 + i * 0.6, 3) * 0.04;
            const angle = baseAngle + flutter + windAngle * 0.07;
            const len = maxPetalLen * (0.22 + bloom * 0.7);
            this.drawSunflowerPetal(ctx, angle, len, petalWidth * 0.92, centerR, bloom, true);
        }

        // Front layer of ray petals
        for (let i = 0; i < petalCount; i++) {
            const baseAngle = (i / petalCount) * Math.PI * 2;
            const flutter = this.noise.get(this.time * 1.15 + i * 0.7, 4) * 0.05;
            const angle = baseAngle + flutter + windAngle * 0.08;
            const len = maxPetalLen * (0.25 + bloom * 0.75);
            this.drawSunflowerPetal(ctx, angle, len, petalWidth, centerR, bloom, false);
        }

        // Center disc
        ctx.save();
        const discGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, centerR);
        discGrad.addColorStop(0, '#6b451f');
        discGrad.addColorStop(0.7, '#3d2712');
        discGrad.addColorStop(1, '#20140a');
        ...
        // Seed texture (spiral dot pattern)
        ctx.shadowBlur = 0;
        const dotCount = 40;
        const golden = 2.399963;
        for (let i = 0; i < dotCount; i++) {
            const a = i * golden + this.time * 0.03;
            const r = centerR * Math.sqrt(i / dotCount) * 0.92;
            const dx = Math.cos(a) * r;
            const dy = Math.sin(a) * r;
            ctx.fillStyle = i % 3 === 0 ? 'rgba(110, 75, 30, 0.65)' : 'rgba(20, 12, 5, 0.6)';
            ctx.beginPath();
            ctx.arc(dx, dy, 1.4 * adjustedScale, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.restore();
    ...
    drawSunflowerPetal(ctx, angle, length, width, centerR, bloom, isBack) {
        ctx.save();
        ctx.rotate(angle);

        const grad = ctx.createLinearGradient(0, -centerR, 0, -centerR - length);
        if (isBack) {
            grad.addColorStop(0, 'hsla(38, 90%, 40%, 0.85)');
            grad.addColorStop(0.5, 'hsla(40, 95%, 48%, 0.85)');
            grad.addColorStop(1, 'hsla(42, 100%, 55%, 0.9)');
        } else {
            grad.addColorStop(0, 'hsla(44, 95%, 48%, 0.92)');
            grad.addColorStop(0.5, 'hsla(42, 100%, 56%, 0.92)');
            grad.addColorStop(1, 'hsla(38, 100%, 62%, 0.96)');
        }
        ctx.fillStyle = grad;
        ctx.shadowBlur = 10 + bloom * 14;
        ctx.shadowColor = `hsla(42, 100%, 60%, ${0.22 + bloom * 0.35})`;

        ctx.beginPath();
        ctx.moveTo(-width * 0.5, -centerR);
        ctx.quadraticCurveTo(-width * 0.62, -centerR - length * 0.55, -width * 0.08, -centerR - length);
        ctx.quadraticCurveTo(0, -centerR - length * 1.06, width * 0.08, -centerR - length);
        ctx.quadraticCurveTo(width * 0.62, -centerR - length * 0.55, width * 0.5, -centerR);
        ctx.closePath();
        ctx.fill();

        // Midrib
        ctx.shadowBlur = 0;
        ctx.strokeStyle = 'hsla(35, 80%, 35%, 0.25)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, -centerR);
        ctx.lineTo(0, -centerR - length * 0.92);
        ctx.stroke();

        ctx.restore();
    }

    // ----- Flower Head (Lily — light pink, 6-petal star, recurved tips) -----
    drawLilyHead(cx, cy, bloom, windAngle, scale) {
        const bloomScaleFactor = 1.0 + bloom * 0.2;
        const adjustedScale = scale * bloomScaleFactor;
        const ctx = this.ctx;
        ...
        if (bloom > 0.02) {
            const glow = ctx.createRadialGradient(0, -glowR * 0.3, 0, 0, -glowR * 0.3, glowR);
            glow.addColorStop(0, `rgba(255, 195, 222, ${0.38 * bloom})`);
            glow.addColorStop(0.5, `rgba(255, 175, 210, ${0.16 * bloom})`);
            glow.addColorStop(1, 'rgba(255, 165, 205, 0)');
            ctx.fillStyle = glow;
            ctx.beginPath();
        ...
        const petalCount = 6;
        const maxLen = 80 * adjustedScale;
        const maxWidth = 20 * adjustedScale;

        // Back triad (alternating) drawn first for depth
        for (let i = 0; i < petalCount; i++) {
            if (i % 2 === 0) continue;
            const baseAngle = (i / petalCount) * Math.PI * 2;
            const flutter = this.noise.get(this.time * 0.9 + i * 0.8, 3) * 0.03;
            const angle = baseAngle * (0.18 + 0.82 * bloom) + flutter + windAngle * 0.07;
            const len = maxLen * (0.32 + 0.68 * bloom);
            this.drawLilyPetal(ctx, angle, len, maxWidth, bloom);
        }
        // Front triad
        for (let i = 0; i < petalCount; i++) {
            if (i % 2 !== 0) continue;
            const baseAngle = (i / petalCount) * Math.PI * 2;
            const flutter = this.noise.get(this.time * 1.05 + i * 0.8, 4) * 0.03;
            const angle = baseAngle * (0.18 + 0.82 * bloom) + flutter + windAngle * 0.07;
            const len = maxLen * (0.35 + 0.65 * bloom);
            this.drawLilyPetal(ctx, angle, len, maxWidth, bloom);
        }
        ...
        if (bloom > 0.18) {
            ...
            const stamenCount = 6;
            for (let i = 0; i < stamenCount; i++) {
                const a = (i / stamenCount) * Math.PI * 2 + this.time * 0.35;
                const curveLen = 24 * adjustedScale * bloom;
                const sx = Math.sin(a) * curveLen * 0.55;
                const sy = -6 - curveLen;

                ctx.strokeStyle = `rgba(255, 235, 240, ${bloom * 0.8})`;
                ctx.lineWidth = 1.3 * adjustedScale;
                ctx.beginPath();
                ctx.moveTo(0, -4);
                ctx.quadraticCurveTo(sx * 0.5, sy * 0.55, sx, sy);
                ctx.stroke();

                ctx.fillStyle = `rgba(165, 65, 100, ${bloom})`;
                ctx.beginPath();
                ctx.ellipse(sx, sy, 2.6 * adjustedScale, 4.6 * adjustedScale, a, 0, Math.PI * 2);
                ctx.fill();
            }
            // Pistil
            ctx.fillStyle = `rgba(255, 245, 248, ${bloom})`;
            ctx.beginPath();
            ctx.arc(0, -10 * adjustedScale, 2.8 * adjustedScale, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
        }
    ...
    drawLilyPetal(ctx, angle, length, width, bloom) {
        ctx.save();
        ctx.rotate(angle);

        // Light pink gradient, near-white toward the throat
        const grad = ctx.createLinearGradient(0, 0, 0, -length);
        grad.addColorStop(0, 'hsla(335, 55%, 94%, 0.85)');
        grad.addColorStop(0.35, 'hsla(336, 68%, 87%, 0.87)');
        grad.addColorStop(0.75, 'hsla(340, 75%, 81%, 0.9)');
        grad.addColorStop(1, 'hsla(344, 78%, 76%, 0.94)');
        ctx.fillStyle = grad;
        ctx.shadowBlur = 10 + bloom * 16;
        ctx.shadowColor = `hsla(340, 90%, 82%, ${0.2 + bloom * 0.35})`;

        // Recurve: tip flares/curls backward as bloom increases
        const recurve = bloom * 0.32;

        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.bezierCurveTo(
            -width * 0.85, -length * 0.35,
            -width * 0.5, -length * (0.86 + recurve * 0.15),
            -width * 0.14, -length * (1 + recurve)
        );
        ctx.quadraticCurveTo(0, -length * (1.05 + recurve * 1.25), width * 0.14, -length * (1 + recurve));
        ctx.bezierCurveTo(
            width * 0.5, -length * (0.86 + recurve * 0.15),
            width * 0.85, -length * 0.35,
            0, 0
        ...
        // Midrib
        ctx.shadowBlur = 0;
        ctx.strokeStyle = 'hsla(340, 55%, 62%, 0.3)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ...
        // Freckles near the throat, a lily hallmark
        if (bloom > 0.3) {
            for (let s = 0; s < 5; s++) {
                const sy = -length * (0.14 + s * 0.055);
                const sx = (s % 2 === 0 ? 1 : -1) * width * 0.14 * (s / 5);
                ctx.fillStyle = `rgba(195, 65, 110, ${0.35 * bloom})`;
                ctx.beginPath();
    ...
    drawPostGlow(cx, cy) {
        ...
        const glowColors =
            this.flowerType === 'sunflower'
                ? ['rgba(255, 190, 80, ', 'rgba(255, 150, 50, ', 'rgba(255, 130, 30, 0)']
                : this.flowerType === 'lily'
                ? ['rgba(255, 190, 220, ', 'rgba(255, 160, 205, ', 'rgba(255, 150, 195, 0)']
                : ['rgba(255, 110, 150, ', 'rgba(255, 70, 110, ', 'rgba(255, 50, 80, 0)'];

        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        const r = 120 + this.bloom * 160;
        const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
        g.addColorStop(0, `${glowColors[0]}${this.bloom * 0.18})`);
        g.addColorStop(0.5, `${glowColors[1]}${this.bloom * 0.08})`);
        g.addColorStop(1, glowColors[2]);
        ctx.fillStyle = g;
        ctx.beginPath();
